...
export default function ClienteInlineEditPanel({
  initial,
  saving = false,
  onCancel,
  onSubmit,
  ...
  useEffect(() => {
    ...
    setError(null);
    // Só reidrata ao montar / trocar cliente (key no pai).
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [initial.nome, initial.telefone, initial.documento]);
  ...
  return (
    <aside
      className={`rounded-[var(--radius-md)] border border-border bg-white shadow-sm overflow-hidden ${className}`}
    >
      <div className="flex items-center justify-between gap-2 px-4 py-3 border-b border-border/70 bg-slate-50/80">
        <div>
          <p className="text-sm font-bold text-foreground">Editar cadastro</p>
          <p className="text-[10px] text-muted-foreground font-medium">
            Alterações à esquerda · ficha à direita
          </p>
        </div>
        <button
          type="button"
          onClick={onCancel}
          disabled={busy}
          className="p-1.5 rounded-[var(--radius-sm)] text-muted-foreground hover:bg-slate-200/80 cursor-pointer disabled:opacity-50"
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-4 max-h-[min(78vh,52rem)] overflow-y-auto">
        ...
        <section className="space-y-2">
          <p className={labelClass}>Imóvel</p>
          <label className="block space-y-1">
            <span className={labelClass}>Tipo</span>
            <select
        ...
        <section className="space-y-2">
          <p className={labelClass}>Comercial</p>
          <div className="grid grid-cols-2 gap-2">
            <label className="space-y-1">
        ...
        <div className="flex gap-2 pt-1 sticky bottom-0 bg-white pb-0.5">
          <Button
            type="button"
            variant="outline"
            disabled={busy}
            onClick={onCancel}
            className="flex-1 text-xs font-bold h-9 rounded-[var(--radius-sm)]"
          >
            Cancelar
          </Button>
          <Button
            type="submit"
            disabled={busy}
            className="flex-1 text-xs font-bold h-9 rounded-[var(--radius-sm)] btn-metallic gap-1.5"
          >
