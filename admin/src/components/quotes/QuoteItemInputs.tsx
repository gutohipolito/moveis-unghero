...
/**
 * Campo de descrição com sugestões de itens salvos.
 * - Permanece livre para digitação.
 * - Ao selecionar uma sugestão, preenche a descrição e os detalhes do item.
 */
      ...
      {showList && (
        ...
        >
          {matches.map((p) => (
            ...
            >
              <span className="block text-xs font-bold text-slate-800">{p.descricao}</span>
              {p.detalhes.length > 0 && (
...
/**
 * Editor de detalhes do item em "chips", com sugestões de detalhes salvos.
...
export function DetailsEditor({
  ...
  const filtered = useMemo(() => {
    ...
    const current = new Set(subitens.map((s) => s.toLowerCase()));
    return suggestions
      .filter((s) => !current.has(s.toLowerCase()) && (q ? s.toLowerCase().includes(q) : true))
      .slice(0, 8);
  }, [draft, suggestions, subitens]);
      ...
      {subitens.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-1.5">
          {subitens.map((s, idx) => (
            <span
              key={`${s}-${idx}`}
              className="inline-flex items-center gap-1 pl-2 pr-1 py-0.5 rounded-md bg-amber-50 border border-amber-200 text-[10px] font-semibold text-amber-800"
            >
              {s}
              <button
                type="button"
                onClick={() => removeDetail(idx)}
                className="p-0.5 rounded hover:bg-amber-100 text-amber-600 cursor-pointer"
        ...
        <div className="flex items-center gap-1.5">
          <div className="relative flex-1">
            ...
            {open && filtered.length > 0 && (
              <div
                className="absolute z-30 left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg max-h-52 overflow-y-auto"
              ...
              >
                {filtered.map((s) => (
                  <button
                    ...
                    className="w-full text-left px-3 py-1.5 text-[11px] font-medium text-slate-700 hover:bg-amber-50 transition-colors border-b border-slate-50 last:border-0 cursor-pointer flex items-center gap-1.5"
                  >
                    <ListPlus className="h-3 w-3 text-amber-500 shrink-0" />
                    {s}
                  </button>
                ))}
              </div>
