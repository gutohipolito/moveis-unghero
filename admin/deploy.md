...
| `ALLOW_ADMIN_BOOTSTRAP` | Deve ficar **ausente** em produção. Só use `true` temporariamente para criar o admin. | — |
| `ALLOW_ADMIN_BOOTSTRAP_PROD` | Em `VERCEL_ENV=production`, também precisa ser `true` além de `ALLOW_ADMIN_BOOTSTRAP`. Remova após o bootstrap. | — |
